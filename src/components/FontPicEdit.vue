    ...
    methods:
      {
        ...
        AddPic(CanvasId,src,x,y,w,h) {
          ...
          main_context.font = 'bold ' + w + 'px 黑体';
          main_context.strokeStyle = 'white';
          main_context.lineWidth = w/12;
          main_context.strokeText(src, x, y);
          main_context.fillStyle = '#3949E3';
          main_context.fillText(src, x, y);
        },
        AddAllPic(CanvasId) {
          var GlobalThis = this;
          var ArrTitle = "";
          ...
          for (let line of ArrTitle) {
            ...
            if (ArrLine.length > 0) {
              ...
              for (let z of ArrLine) {
                this.AddPic(CanvasId, z, GlobalThis.TitleX, GlobalThis.TitleY,GlobalThis.TitleW, GlobalThis.TitleW*GlobalThis.TitleWHRate);
          ...
          for (let line of ArrName) {
            if  (line==""){continue}
            var ArrLine = line.split("");
            GlobalThis.NameX = (1242 - (GlobalThis.NameW * ArrLine.length + GlobalThis.NameWS * (ArrLine.length-1))) / 2;
            if (ArrLine.length > 0) {
              //console.log(GlobalThis.TitleX+";"+ArrLine.length);
              for (let z of ArrLine) {
                this.AddPic(CanvasId, z, GlobalThis.NameX, GlobalThis.NameY,GlobalThis.NameW, GlobalThis.NameW*GlobalThis.TitleWHRate);
                GlobalThis.NameX = GlobalThis.NameX + GlobalThis.NameW + GlobalThis.NameWS;
              }
              GlobalThis.TitleX = GlobalThis.TitleIX;
